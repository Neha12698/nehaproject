...
<body>
  ...
  if(isset($_SESSION["id"])) {
     ...
     {
       // $ed_sel="select * from users where id='$_REQUEST[id]'";
    $ed_sel="select u.first_name,u.last_name,u.email,u.img,u.hob,u.gen,u.state_id,u.user_type,c.id,c.con_name,s.con_id,s.state_name,s.con_id from users as u INNER JOIN country as c INNER JOIN state as s ON u.state_id=s.id AND c.id=s.con_id where u.id='$_REQUEST[id]'";
...
<div class="container">
  ...
  <div class="form-group">
    ...
    <input type="text" name="eml" placeholder="Enter email address" class="form-control" value="<?php if(isset($_POST['eml'])) echo $_POST['eml']; else  echo $fetch_data['email']; ?>" disabled="disabled"></br>
     ...
     <input type="hidden" name="mode"  class="form-control" value="edit_admin">
  </div>
  <div class="form-group">
  ...
  <div class="form-group">
   ...
   <div class="form-group">
    <lable for="country" class="col-sm-3 control-label">User Type</lable>
    <div class="col-sm-9">
      <?php 

           $fetch_type=$fetch_data['user_type'];
         
      ?>
    <select name="type" class="form-control">
    <option value="">--select type--</option>
    <option  <?php if(isset($fetch_type)) if($fetch_type=="user"){echo "selected='selected'";}?>>user</option>
    <option  <?php if(isset($fetch_type)) if($fetch_type=="admin"){echo "selected='selected'";}?>>admin</option>
    </select>
    <span class = "error"><?php if(isset($error['con'])) echo $error['con']; ?></span>
    </div>
  </div><br>
